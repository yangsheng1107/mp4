...
int main(int argc, char *argv[])
{
    ...
    second = (duration / (qt->moov.trak[0]->mdia.mdhd.time_scale));
    printf("video duration = %d \r\n", second);

    second = ((qt->moov.trak[0]->mdia.mdhd.duration) / (qt->moov.trak[0]->mdia.mdhd.time_scale));
    printf("video duration = %d \r\n", second);
    printf("video fps = %f \r\n", ((float)(qt->moov.trak[0]->mdia.minf.stbl.stts.table->sample_count) / (float)second));

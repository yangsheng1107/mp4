...
int quicktime_set_iod_audio_profile_level(quicktime_t *file, int id)
{
    ...
    return quicktime_iods_set_audio_profile(&file->moov.iods, id);
}
...
int quicktime_set_iod_video_profile_level(quicktime_t *file, int id)
{
    ...
    return quicktime_iods_set_video_profile(&file->moov.iods, id);
}
...
int quicktime_add_audio_sdp(quicktime_t *file, char* sdpString, int track, int hintTrack)
{
    ...
    return quicktime_sdp_append(&(hintTrak->hint_udta.hnti.sdp), sdpString);
}
...
int quicktime_write_audio_frame(quicktime_t *file, unsigned char *audio_buffer, long bytes, int track)
{
    ...
    result = quicktime_write_data(file, (char *)audio_buffer, bytes);
                                      ...
                                      u_char isSyncSample)
{
    ...
    /* write the hint data */
    result = quicktime_write_data(file, (char *)hintBuffer, hintBufferSize);
    result = (result ? 0 : 1);
...
long quicktime_read_audio_frame(quicktime_t *file, unsigned char *audio_buffer,  int maxBytes, int track)
{
    ...
    result = quicktime_read_data(file, (char *)audio_buffer, bytes);
...
int quicktime_has_keyframes(quicktime_t *file, int trackid)
{
    quicktime_trak_t *trak = file->vtracks[trackid].track;
...
{    
    ...
    if (quicktime_match_32(codec_id, QUICKTIME_H264))
    {
        ...
        a = (unsigned char)data[0 + 4 + len];
        b = (unsigned char)data[1 + 4 + len];
        c = (unsigned char)data[2 + 4 + len];
        d = (unsigned char)data[3 + 4 + len];
        len2 = (a<<24) | (b<<16) | (c<<8) | d;
        if ((data[4 + len + 4 + len2 + 4] & 31) == 5)
        {
            return 1;
        }
        else
